    ...
    <div class="container px-4 md:px-6">
      ...
      <div class="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
        <BoardCard
          ...
          :key="board.id"
          :board="board"
          @click.native="openModal(board)"
        />
      </div>
      <pagination :current-page="currentPage" :total-pages="totalPages" @update:page="updatePage" />
    </div>
    <DetailModal :board="selectedBoard" :isOpen="modalOpen" @close="modalOpen = false" />
  </section>
  <RouterView />
</template>
...
import DetailModal from "@/components/board/BoardDetail.vue";
